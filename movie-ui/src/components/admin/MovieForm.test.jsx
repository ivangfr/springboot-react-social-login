...
describe('MovieForm', () => {
  ...
  it('disables Create button when imdb is empty but title is filled', () => {
    render(
      <MovieForm {...makeProps({ movieImdb: '', movieTitle: 'Some Title' })} />
    )
    expect(screen.getByRole('button', { name: /create/i })).toBeDisabled()
  })
  ...
  it('disables Create button when title is empty but imdb is filled', () => {
    render(
      <MovieForm {...makeProps({ movieImdb: 'tt1234567', movieTitle: '' })} />
    )
    expect(screen.getByRole('button', { name: /create/i })).toBeDisabled()
  })
  ...
  it('disables Create button when imdb is whitespace only', () => {
    render(
      <MovieForm
        {...makeProps({ movieImdb: '   ', movieTitle: 'Some Title' })}
      />
    )
    expect(screen.getByRole('button', { name: /create/i })).toBeDisabled()
  })
  ...
  it('enables Create button when both imdb and title are provided', () => {
    render(
      <MovieForm
        {...makeProps({ movieImdb: 'tt1234567', movieTitle: 'The Matrix' })}
      />
    )
  ...
  it('does not call handleAddMovie when button is disabled', async () => {
    ...
    render(
      <MovieForm
        {...makeProps({ movieImdb: '', movieTitle: '', handleAddMovie })}
      />
    )
  ...
  it('calls handleInputChange when user types in IMDB field', () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/imdb/i), {
      target: { value: 'tt' }
    })
