...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
  ...
  public CustomOAuth2UserService(
      UserService userService, List<OAuth2UserInfoExtractor> oAuth2UserInfoExtractors) {
            ...
            .orElseThrow(
                () ->
                    new InternalAuthenticationServiceException(
                        "The OAuth2 provider is not supported yet"))
  ...
  private User upsertUser(CustomUserDetails customUserDetails) {
    User user =
        userService
            .getUserByUsername(customUserDetails.getUsername())
            .map(
                existing -> {
