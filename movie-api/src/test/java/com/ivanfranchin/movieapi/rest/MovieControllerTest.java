...
class MovieControllerTest {
  ...
  @MockitoBean CustomAuthenticationSuccessHandler customAuthenticationSuccessHandler;
  ...
  void getMovies_withText_asAdmin_returns200() throws Exception {
    when(movieService.getMoviesContainingText("rocky"))
        .thenReturn(List.of(createMovie("tt001", "Rocky")));

    mockMvc
        .perform(get("/api/movies").param("text", "rocky"))
        .andExpect(status().isOk())
  ...
  void getMovies_withBlankText_returnsAllMovies() throws Exception {
    ...
    mockMvc
        .perform(get("/api/movies").param("text", "  "))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$[0].imdb").value("tt001"))
        .andExpect(jsonPath("$[0].title").value("Alpha"));
  }
  ...
  void deleteMovie_asAdmin_movieExists_returns204() throws Exception {
    ...
    mockMvc.perform(delete("/api/movies/tt001")).andExpect(status().isNoContent());
  }

  @Test
  @WithMockUser(authorities = "ADMIN")
  void deleteMovie_asAdmin_notFound_returns404() throws Exception {
    when(movieService.validateAndGetMovie("tt999"))
        .thenThrow(new MovieNotFoundException("Movie with imdb tt999 not found"));

    mockMvc.perform(delete("/api/movies/tt999")).andExpect(status().isNotFound());
  }

  @Test
  @WithMockUser(authorities = "USER")
  void deleteMovie_asUser_returns403() throws Exception {
    mockMvc.perform(delete("/api/movies/tt001")).andExpect(status().isForbidden());
  }

  @Test
  void deleteMovie_unauthenticated_returns401() throws Exception {
    mockMvc.perform(delete("/api/movies/tt001")).andExpect(status().isUnauthorized());
  }
